...
const ArrowButton: React.FC<{ direction: 'left' | 'right'; onClick: () => void; disabled: boolean; }> = ({ direction, onClick, disabled }) => (
    <motion.button
        onClick={onClick}
        disabled={disabled}
        className="w-12 h-12 rounded-full bg-white/10 border border-white/20 backdrop-blur-sm flex items-center justify-center disabled:opacity-30 disabled:cursor-not-allowed transition-opacity duration-300"
...
const Timeline: React.FC = () => {
    ...
    return (
        <div className="relative h-screen w-full bg-[#0c1015]">
            <div className="sticky top-0 h-screen w-full overflow-hidden bg-[#0c1015]">
                ...
                <div className="relative mx-auto flex h-full w-full max-w-5xl flex-col items-center justify-center pt-24 lg:flex-row lg:pt-12">
                    
                    {/* desktop prev */}
                    <div className="absolute left-0 hidden lg:block z-20">
                    ...
                    <div className="flex flex-1 flex-col justify-between lg:w-1/2 lg:justify-center">
                        <div className="relative flex flex-grow items-center justify-center text-center lg:text-left order-2 lg:order-none -translate-y-4 z-10">
                            {/* mobile nav */}
                            <div className="lg:hidden absolute inset-x-0 top-1/2 -translate-y-1/2 flex items-center justify-between px-4 z-20">
                                <ArrowButton direction="left" onClick={handlePrevious} disabled={currentStep === 0} />
                                <ArrowButton direction="right" onClick={handleNext} disabled={currentStep === totalSteps} />
                            </div>

                            {timelineData.map((item, index) => (
                                <motion.div key={item.week} className="absolute w-full max-w-lg px-20 lg:px-12" initial={{ opacity: 0, y: 40, scale: 0.9 }} animate={{ opacity: currentStep === index + 1 ? 1 : 0, y: currentStep === index + 1 ? 0 : (currentStep > index + 1 ? -40 : 40), scale: currentStep === index + 1 ? 1 : 0.9, pointerEvents: currentStep === index + 1 ? 'auto' : 'none' }} transition={{ duration: 0.6, ease: [0.25, 0.1, 0.25, 1], type: 'spring', stiffness: 200, damping: 25 }}>
                    ...
                    {/* desktop next */}
                    <div className="absolute right-0 hidden lg:block z-20">
                        <ArrowButton direction="right" onClick={handleNext} disabled={currentStep === totalSteps} />
                    </div>
                </div>
            </div>
